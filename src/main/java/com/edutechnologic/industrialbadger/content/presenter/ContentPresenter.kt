package com.edutechnologic.industrialbadger.content.presenter

import android.os.Bundle
import android.util.Log
import androidx.fragment.app.FragmentActivity
import androidx.lifecycle.Observer
import com.edutechnologic.industrialbadger.base.util.html
import com.edutechnologic.industrialbadger.content.contract.ContentContract
import com.edutechnologic.industrialbadger.content.data.ContentModuleState
...
class ContentPresenter(
    ...
    override fun onViewCreated() {
        when (state.getContext()) {
            ContentContract.Ctx.INDEX -> lifecycleOwnerRef.get()?.let { _viewmodel.onIndex(it) }
            ContentContract.Ctx.DETAILS -> lifecycleOwnerRef.get()?.let { owner ->
                state.getInitialContent()?.let { _viewmodel.onDetails(owner, it) }
            }
            ContentContract.Ctx.TRAINING -> TODO()
            ContentContract.Ctx.SEARCH -> TODO()
        }
    }
